...
const getCart = async (setProceed, setCart, authToken) => {
  if (setProceed) {
    const { data } = await axios.get(
      `${process.env.REACT_APP_SERVER_URL}/cart/fetchcart`,
      {
        headers: {
...
const getWishList = async (setProceed, setWishlistData, authToken) => {
  if (setProceed) {
    const { data } = await axios.get(
      `${process.env.REACT_APP_SERVER_URL}/wishlist/fetchwishlist`,
      {
        headers: {
          Authorization: authToken,
        },
...
const getAllProducts = async (setData) => {
  try {
    const { data } = await axios.get(
      `${process.env.REACT_APP_SERVER_URL}/product/fetchproduct`
    );
    setData(data);
  } catch (error) {
    console.log(error);
...
const getSingleProduct = async (setProduct, id, setLoading) => {
  const { data } = await axios.get(
    `${process.env.REACT_APP_SERVER_URL}/product/fetchproduct/${id}`
  );
  setProduct(data);
  setLoading(false);
};
